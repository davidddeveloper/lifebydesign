...
function scoreLevers(d: any) {
  ...
  // Lever 1 — WHO: Q1(×1) Q2(×2) Q3(×1) / max 16
  const lever1 = parseFloat((((n(d.q1) + n(d.q3)) + n(d.q2) * 2) / 16 * 10).toFixed(1))

  // Lever 2 — WHAT: Q5+Q6+Q9+Q11(×1) + Q7(×1.5) + Q8+Q10(×2) / max 40
  const lever2 = parseFloat((
    ((n(d.q5) + n(d.q6) + n(d.q9) + n(d.q11)) + n(d.q7) * 1.5 + (n(d.q8) + n(d.q10)) * 2)
    / 40 * 10
  ).toFixed(1))

  // Lever 3 — FIND YOU: Q13+Q16+Q17(×1) + Q14(×1.5) + Q15(×2) / max 28
  const lever3 = parseFloat((
    ((n(d.q13) + n(d.q16) + n(d.q17)) + n(d.q14) * 1.5 + n(d.q15) * 2)
    / 28 * 10
  ).toFixed(1))

  // Lever 4 — SELL: Q18(×1.5) + Q19+Q21(×2) + Q20(×1) / max 26
  const lever4 = parseFloat((
    (n(d.q18) * 1.5 + (n(d.q19) + n(d.q21)) * 2 + n(d.q20))
    / 26 * 10
  ).toFixed(1))

  // Lever 5 — DELIVER: Q23+Q24(×1) + Q27(×1.5) + Q25+Q26(×2) / max 30
  const lever5 = parseFloat((
    ((n(d.q23) + n(d.q24)) + n(d.q27) * 1.5 + (n(d.q25) + n(d.q26)) * 2)
    / 30 * 10
  ).toFixed(1))
...
export async function POST(request: NextRequest) {
  try {
    ...
    const constraint = identifyConstraint(rawScores)
    ...
    const recommendedCta = selectCta(rawScores, constraint.ruleApplied)
    ...
    const { data: saved, error: saveError } = await supabaseAdmin
      ...
      .insert({
        ...
        q23: body.q23, q24: body.q24, q25: body.q25, q26: body.q26, q27: body.q27,
        // Open text
