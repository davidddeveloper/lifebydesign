      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          >
            ...
            <span className="text-orange-600">Business Audit</span>
          </motion.h1>
          <p className="text-xl text-gray-600 mb-2">
            Identify the ONE thing holding you back from Le 125M ($50K) monthly revenue
          </p>
          <p className="text-gray-500">
            ⏱️ Takes 15 minutes • 🎯 Get instant results
          </p>
        </div>
          ...
          <div className="flex justify-between mt-2">
            {steps.map((step, index) => (
              <div
                key={index}
                className={`text-xs ${
                  index <= currentStep ? 'text-orange-600 font-semibold' : 'text-gray-400'
                }`}
              >
                {step.title}
              </div>
        ...
        >
          ...
          <AnimatePresence mode="wait">
            ...
            {currentStep === 1 && (
              <WhoStep formData={formData} onChange={handleInputChange} />
            )}
            {currentStep === 2 && (
              <WhatStep formData={formData} onChange={handleInputChange} />
            )}
            {currentStep === 3 && (
              <SellStep formData={formData} onChange={handleInputChange} />
            )}
            {currentStep === 4 && (
              <TrafficStep formData={formData} onChange={handleInputChange} />
            )}
            {currentStep === 5 && (
              <DeliverStep formData={formData} onChange={handleInputChange} />
            )}
            {currentStep === 6 && (
              <FinalStep formData={formData} onChange={handleInputChange} />
            )}
          </AnimatePresence>
          ...
          <div className="flex justify-between mt-12 pt-8 border-t border-gray-200">
            ...
            {currentStep === steps.length - 1 ? (
              <button
                ...
                className="px-8 py-3 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-lg font-bold text-lg hover:from-orange-700 hover:to-orange-600 transition-all transform hover:scale-105 shadow-lg"
              >
                🎯 Get My Results
              </button>
            ) : (
              <button
                onClick={handleNext}
                className="px-8 py-3 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-lg font-semibold hover:from-orange-700 hover:to-orange-600 transition-all"
              >
                Next →
              </button>
            )}
...
function FinalStep({ formData, onChange }: any) {
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-br from-orange-50 to-amber-50 border-2 border-orange-200 rounded-xl p-6 mt-8">
        <h3 className="text-xl font-bold text-gray-900 mb-3">
          🎉 You're Almost Done!
        </h3>
        <p className="text-gray-700 mb-4">
          In the next screen, you'll discover:
        </p>
...
function FormField({ label, value, onChange, placeholder = '', type = 'text', multiline = false, required = false, helpText = '' }: any) {
  return (
    <div>
